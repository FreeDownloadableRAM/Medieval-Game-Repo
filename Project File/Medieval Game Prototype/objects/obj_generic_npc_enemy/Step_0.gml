	...
	if (in_attack_anim == true){
		...
		if (global.enemy_order == 0){
			...
			if (instance_exists(retreat_rally_target)){
				...
				if (x > (retreat_rally_target.x - ai_rally_distance_offset - ai_movement_margin_of_error_allowed) && x < (retreat_rally_target.x - ai_rally_distance_offset + ai_movement_margin_of_error_allowed)){
				...
				// You havent reached your movement target
					...
					if (x > retreat_rally_target.x - ai_rally_distance_offset){
					...
					else if (x < retreat_rally_target.x - ai_rally_distance_offset){
		...
		// Defend
			...
			if (instance_exists(defend_rally_target)){
				...
				if (instance_exists(enemy_target)){
					...
					if (distance_to_object(_closest_enemy) > character_vision_distance && _closest_enemy.x > defend_rally_target){
						// Debugging
					...
					else if (distance_to_object(_closest_enemy) < character_vision_distance && _closest_enemy.x > defend_rally_target && distance_to_object(defend_rally_target) < 64){
						// Debugging
					...
					// situation D and C need work, on defense order, they will chase targets nearby when not near rally or behind rally point	
					// what if the enemy is in vision and is on top of or to the right of the rally point?
					else if ((distance_to_object(_closest_enemy) <= character_vision_distance) && (_closest_enemy.x >= defend_rally_target.x)){
		...
		// Attack
			...
			if (instance_exists(enemy_target)){
				...
				// Enemies but no castle
					...
					// as thinks enemy is behind king, even though they are in front.
					// WIP Attack Start ==============================================================================
